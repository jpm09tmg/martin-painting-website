...
export default function QuotePage() {
  ...
  const locationTypes = [
    { name: 'Interior', description: 'Indoor painting projects', baseRate: 3.0 },
    { name: 'Exterior', description: 'Outdoor painting projects', baseRate: 3.5 }
  ]
  ...
  const paintConditions = [
    ...
    { name: 'Poor', description: 'Extensive prep work needed', multiplier: 1.15 }
  ]
  ...
  const calculateEstimate = () => {
    ...
    const locationType = locationTypes.find(type => type.name === formData.locationType)
    let baseRate = locationType?.baseRate || 3.0

    // multipliers are applied based on selections

    const propertyMultiplier = propertyTypes.find(type => type.name === formData.propertyType)?.multiplier || 1.0
    const conditionMultiplier = paintConditions.find(condition => condition.name === formData.currentCondition)?.multiplier || 1.2
    const qualityMultiplier = paintQualities.find(quality => quality.name === formData.paintQuality)?.multiplier || 1.0
    
    // ceiling height adjustments
    let ceilingMultiplier = 1.0
    if (formData.ceilingHeight === '9ft') ceilingMultiplier = 1.05
    if (formData.ceilingHeight === '10ft') ceilingMultiplier = 1.10
    if (formData.ceilingHeight === '10ft+') ceilingMultiplier = 1.15

    // calculate base cost
    let totalCost = sqft * baseRate * propertyMultiplier * conditionMultiplier * qualityMultiplier * ceilingMultiplier
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                      ...
                      {locationTypes.map((location) => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-xs text-[#74A744] font-medium">${location.baseRate}/sq ft base</div>
                          </div>
                        </label>
                      ))}
                    </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-lg border border-gray-200 sticky top-8">
              <div className="p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Your Estimate</h2>
                  ...
                  <div>
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="pt-3 border-t">
                        <div className="text-xs text-gray-500 mb-2">Applied adjustments:</div>
                        {estimate.breakdown.propertyMultiplier !== 1.0 && (
                          <div className="flex justify-between text-xs">
                            <span>Property type:</span>
                            <span>{Math.round((estimate.breakdown.propertyMultiplier - 1) * 100)}%</span>
                          </div>
                        )}
                        {estimate.breakdown.conditionMultiplier !== 1.0 && (
                          <div className="flex justify-between text-xs">
                            <span>Paint condition:</span>
                            <span>{Math.round((estimate.breakdown.conditionMultiplier - 1) * 100)}%</span>
                          </div>
                        )}
                        {estimate.breakdown.qualityMultiplier !== 1.0 && (
                          <div className="flex justify-between text-xs">
                            <span>Paint quality:</span>
                            <span>{Math.round((estimate.breakdown.qualityMultiplier - 1) * 100)}%</span>
                          </div>
                        )}
                        {estimate.breakdown.ceilingMultiplier !== 1.0 && (
                          <div className="flex justify-between text-xs">
                            <span>Ceiling height:</span>
                            <span>{Math.round((estimate.breakdown.ceilingMultiplier - 1) * 100)}%</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="text-xs text-blue-700">
                    <strong>Note:</strong> This is a preliminary estimate only. Final pricing may vary based on site conditions and specific requirements.
                  </div>
                </div>
              </div>
            </div>
